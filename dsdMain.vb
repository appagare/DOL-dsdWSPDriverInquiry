...
Public Class svrMain
    ...
    Private QueueRxName As String = "PRIVATE$\Driver" 'name of the input queue
    ...
    Private Const REGION_PREFIX As String = "DQR2." 'Unisys requires regional codes that are not passed over the TCP/IP line
...
#Region " Component Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        'svrMain
        '
        Me.ServiceName = "WSP Driver Inquiry"
...
#End Region
    ...
    Private Sub ProcessMessage(ByVal State As Object)
        ...
        Dim MAXSO As New gov.wa.dol.dolservicepredev.dolWSPservice()

        Try

            'if we have credentials, assign them to the web service request
            If Not Credentials Is Nothing Then
                MAXSO.Credentials = Credentials
            End If
            ...
            'set PreAuthenticate
            MAXSO.PreAuthenticate = PreAuthenticate

            Dim IsDNQ As Boolean = False
            Dim IsDQG As Boolean = False

            Select Case Left(WspRequest.Body, 4)
                Case "DNQ."
                    'NLETS name only - must respond with DNR
                    IsDNQ = True
                    'strResponse = MAXSO.CallMAXSO(REGION_PREFIX & WspRequest.Body)
                Case "DQG."
                    'NLETS regional inquiry - must respond with DRG
                    IsDQG = True
                    'strResponse = MAXSO.CallMAXSO(REGION_PREFIX & WspRequest.Body)
                Case Else
                    'in-state or normal NLETS
                    'strResponse = MAXSO.CallMAXSO(REGION_PREFIX & WspRequest.Body)
            End Select
            'make the call
            strResponse = MAXSO.CallMAXSO(REGION_PREFIX & WspRequest.Body)

            'the response string contains LF LF rather than CR LF
            'remove any CRs, convert LF LF to LF, then expand LFs to CRLFs
            strResponse = Replace(Replace(Replace(strResponse, vbCr, ""), vbLf & vbLf, vbLf), vbLf, vbCrLf)

            'strip the region portion of the response per WSP 7-28-2004
            strResponse = StripRegion(strResponse)

            'if request was a DNQ, response must be DNR
            If IsDNQ = True Then
                'convert NLETS DR. response back to DNR. response if inquiry was an DNQ
                strResponse = Replace(strResponse, "DR.", "DNR.", 1, 1)
            ElseIf IsDQG = True Then
                'convert NLETS DR. response back to DRG. response if inquiry was an DQG
                strResponse = Replace(strResponse, "DR.", "DRG.", 1, 1)
            End If
        ...
        Catch ex As Exception
            ...
            Dim strMessage As String = "Error from MAXSO" & vbCrLf & "Request: " & WspRequest.Body & vbCrLf & vbCrLf & "Error: " & ex.Message
            If Not ex.InnerException Is Nothing Then
                strMessage &= vbCrLf & "Inner Exception: " & ex.InnerException.Message
            End If
        ...
        If Trim(strResponse) = "" Then
            ...
            LogEvent("ProcessMessage", "Empty response from MAXSO", MessageType.Error, LogType.Standard)
            strResponse = strUNAVAILABLE
        ElseIf Left(strResponse, 2) = "s!" Or Left(strResponse, 2) = "!!" Then
            'handle errors from MAXSO ("s!..." or "!!...")
        ...
        While ServiceState = ServiceStates.Running
            ...
            Catch ex As Exception
                ...
                If DebugMode > 0 Then
                    ...
                    LogEvent("ProcessQueue", "Err: " & strMessage, MessageType.Debug, LogType.Standard)
                End If
    ...
    Private Function StripRegion(ByVal Response As String) As String
        'The Unisys returns regional information in the response that cannot be passed 
        'to the ACCESS switch when using the TCP/IP connection. This function 
        'strips these regional characters from the response.

        Select Case Left(Response, 3)
            Case "DQ.", "KQ.", "D .", "K .", "DW.", "DR.", "DRG", "DNR", "KR."
